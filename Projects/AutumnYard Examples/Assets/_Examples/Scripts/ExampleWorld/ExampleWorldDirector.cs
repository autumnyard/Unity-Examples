...
namespace AutumnYard.ExampleWorld
{
    public sealed class ExampleWorldDirector : MonoBehaviour
    {
        ...
        [SerializeField] private UI.ExampleWorldUI _ui;

        private void Awake()
        {
            if (_ui == null) _ui = FindObjectOfType<UI.ExampleWorldUI>();

            SceneHandler.Instance.ForceSetCurrentContext(SceneHandler.Context.ExampleWorld);
        ...
        private void ChangeMode_ToUI(UI.ExampleWorldUI.Menu menuToOpen)
        {
            ...
            switch (menuToOpen)
            {
                case UI.ExampleWorldUI.Menu.Pause:
                    _ui.OpenPause();
                    break;
            }
        ...
        public void Button_OpenPause() => ChangeMode_ToUI(UI.ExampleWorldUI.Menu.Pause);
