...
namespace AutumnYard.ExampleCards
{
    public sealed class ExampleCardsDirector : MonoBehaviour
    {
        ...
        [SerializeField] private UI.ExampleCardsUI _ui;

        private void Awake()
        {
            if (_ui == null) _ui = FindObjectOfType<UI.ExampleCardsUI>();

            SceneHandler.Instance.ForceSetCurrentContext(SceneHandler.Context.ExampleCards);
        ...
        private void ChangeMode_ToUI(UI.ExampleCardsUI.Menu menuToOpen)
        {
            ...
            switch (menuToOpen)
            {
                case UI.ExampleCardsUI.Menu.Pause:
                    _ui.OpenPause();
                    break;
            }
        ...
        public void Button_OpenPause() => ChangeMode_ToUI(UI.ExampleCardsUI.Menu.Pause);
